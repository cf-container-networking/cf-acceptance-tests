...
var _ = Describe(deaUnsupportedTag+"SSH", func() {
	...
	Context("a multi-instance app", func() {
		BeforeEach(func() {
	...
	Context("a single-instance app", func() {
		BeforeEach(func() {
			appName = generator.PrefixedRandomName("CATS-APP-")
			Eventually(cf.Cf(
				"push", appName,
		...
		Describe("ssh", func() {
			It("can execute a remote command in the container", func() {
				envCmd := cf.Cf("ssh", "-i", "1", appName, "-c", "/usr/bin/env && /usr/bin/env >&2", "-vvv")
			...
			It("runs an interactive session when no command is provided", func() {
				envCmd := exec.Command("cf", "ssh", "-i", "1", appName, "-vvv")
			...
			It("allows local port forwarding", func() {
				listenCmd := exec.Command("cf", "ssh", "-i", "1", "-L", "127.0.0.1:61007:localhost:8080", appName, "-vvv")
